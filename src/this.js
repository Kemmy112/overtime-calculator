function toCompute() {
  ...
  if (workingHours <= 0 || hoursPay <= 0) {
    document.getElementsByClassName("result")[0].innerHTML =
      "Hours and pay rate must be positive!";
    return;
  }
  ...
  // Update result
  document.getElementsByClassName("result")[0].innerHTML = `$${totalTip.toFixed(
    2
  )}`;
}

document.getElementById("dobtn").addEventListener("click", toCompute);
